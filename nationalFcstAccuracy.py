...
_from_date='20150201' ;_to_date='20150228' ; fcs_ldays=8 ;
...
for date in rrule.rrule(rrule.DAILY,dtstart=fcs_from_date,until=fcs_to_date):
         ...
         #modFle="/home/Data/poolLatestWorkByModelTeam/output/correctedFile/feb2015/24hourlycorrectedHeatIndex"+date+".csv" ; 
         #modFle="/home/Data/poolLatestWorkByModelTeam/nationalCor/correctedFcst/3daybiasCorfle"+date+"00.csv" ;         
         modFle="/home/Data/poolLatestWorkByModelTeam/output/uncorrectedFile/24hourly"+date+"00.csv" ; 
...
C_r=np.zeros((nat_idd.shape[0],7)).astype(int)
...
maxrh=np.concatenate([locs,np.vstack(rhwscnt_day1[:,0]),rhwscnt_day1[:,1::2],rhwscnt_day2[:,1::2],rhwscnt_day3[:,1::2],rhwscnt_day4[:,1::2],rhwscnt_day5[:,1::2],rhwscnt_day6[:,1::2],rhwscnt_day7[:,1::2],C_r],axis=1)
maxrh=maxrh[maxrh[:,0].argsort(kind='mergesort')] ;
minrh=np.concatenate([locs,np.vstack(rhwscnt_day1[:,0]),rhwscnt_day1[:,2::2],rhwscnt_day2[:,2::2],rhwscnt_day3[:,2::2],rhwscnt_day4[:,2::2],rhwscnt_day5[:,2::2],rhwscnt_day6[:,2::2],rhwscnt_day7[:,2::2],C_r],axis=1)
...
rf_day1=np.concatenate([mod[:,0::7,0:3],mod[:,0::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,0::7,0:2],obs[:,0::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day2=np.concatenate([mod[:,1::7,0:3],mod[:,1::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,1::7,0:2],obs[:,1::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day3=np.concatenate([mod[:,2::7,0:3],mod[:,2::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,2::7,0:2],obs[:,2::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day4=np.concatenate([mod[:,3::7,0:3],mod[:,3::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,3::7,0:2],obs[:,3::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day5=np.concatenate([mod[:,4::7,0:3],mod[:,4::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,4::7,0:2],obs[:,4::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day6=np.concatenate([mod[:,5::7,0:3],mod[:,5::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,5::7,0:2],obs[:,5::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
rf_day7=np.concatenate([mod[:,6::7,0:3],mod[:,6::7,8].reshape(nat_idd.shape[0],no_fcs_days,1),obs[:,6::7,0:2],obs[:,6::7,6].reshape(nat_idd.shape[0],no_fcs_days,1)],axis=2)  
...
np.savetxt('nationalUncorrected01-07daysFcstAccuracy.csv', acc_trwr,fmt="%s",delimiter=",") ;
